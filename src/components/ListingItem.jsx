...
export default function ListingItem({listing, id, onEdit, onDelete}) {
  return <li className="relative bg-white flex flex-col justify-between items-center shadow-md 
  hover:shadow-xl rounded-md overflow-hidden transition-shadow duration-150 m-[10px]">
    <Link className= "contents" to={`/category/${listing.type}/${id}`}>
      ...
      <div className="w-full p-[10px] ">
        <div className="flex items-center space-x-1">
          ...
          <p className="font-semibold text-sm mb-[2px] text-gray-800 truncate">{listing.address}</p>
        </div>
        <p className="font-semibold mt-0 text-xl truncate">{listing.name}</p>
        <p className="font-semibold mt-2  text-[#457b9d]">${listing.offer 
    ...
    {onEdit && (
      <MdEdit className="absolute bottom-2 right-7 h-4 cursor-pointer text-black-500" onClick={()=> onEdit(listing.id)}/> 
    )}
